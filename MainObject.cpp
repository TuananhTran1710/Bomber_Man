...
void MainObject::RemoveBullet_Min(const int& idx)
{
	int size = bullet_list_min.size();
	if (size > 0 && idx < size)
	{
		BulletObject* p_bullet = bullet_list_min.at(idx);
		num_min--;
		if (num_min == 0)
		{
			max_bom = 1;
		...
		bullet_list_min.erase(bullet_list_min.begin() + idx);
		if (p_bullet)
		{
			delete p_bullet;
			p_bullet = NULL;
